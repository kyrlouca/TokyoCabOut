...
uses
  ...
  Wwdbcomb, Data.DB, DBAccess, vcl.wwdblook, vcl.Wwdotdot, vcl.wwbutton,kyriacosTypes,G_kyriacosTypes;
...
type
  TH_FlightairwaybillFRM = class(TForm)
    ...
    ItemsGRDIButton: TwwIButton;
    ServiceFLD: TwwDBComboBox;
...
uses MainForm,H_ScanAirwaybill, S_SelectCustomerx, S_SelectTariffY,
   G_KyrSQL, G_generalProcs, H_FlightAirItem, M_customerNew;
...
ksfillComboF1(cn,ServiceFLD,'service_code','Code','Code','Code',false,false);
ksfillComboF1(cn,PayMethodFLD,'PAYMENT_METHOD','THE_KEY','DESCRIPTION','THE_KEY',false,false);

ksOpenTables([CountrySQL,currencySQL,ItinerarySQL,IncotermsSQL, GroupCertificateSQL,CertItemSQL,SelectCertSQL,Fa_ItemSQL]);
...
var
  Serial:Integer;
begin
  Serial:=FA_ItemSQL.FieldByName('Serial_number').AsInteger;
...
procedure TH_FlightairwaybillFRM.ItemsGRDTitleButtonClick(Sender: TObject;
  AFieldName: string);
var
...
begin
        HawbDS:=TIBCQUery(ItemsGRD.DataSource.DataSet);
...
procedure TH_FlightairwaybillFRM.wwNavButton4Click(Sender: TObject);
var
  ItemSerial:integer;
begin
  ItemSerial:=Fa_ItemSQL.FieldByName('serial_number').AsInteger;
  if ItemSerial>0 then begin
    ksExecSQLVar(cn, 'delete from FLIGHT_AIRWAYBILL_ITEM_CERT where FK_FLIGHT_AIRWAYBILL_ITEM= :item',[ItemSerial]);
    ksExecSQLVar(cn, 'delete from FLIGHT_AIRWAYBILL_ITEM where serial_number = :item',[ItemSerial]);
  end;
  fa_ItemSQL.Refresh;
  abort;
end;
