object M_SpecificCircumsatnceFRM: TM_SpecificCircumsatnceFRM
  Left = 483
  Top = 710
  Caption = 'Declaration Typ'
  ClientHeight = 469
  ClientWidth = 753
  Color = clBtnFace
  ...
  object TableSQL: TIBCQuery
    SQLInsert.Strings = (
      'INSERT INTO AUX_SPECIFIC_CIRCUMSTANCE'
      '  (IS_DEFAULT, DESCRIPTION, KEY)'
      'VALUES'
      '  (:IS_DEFAULT, :DESCRIPTION, :KEY)')
    SQLDelete.Strings = (
      'DELETE FROM AUX_SPECIFIC_CIRCUMSTANCE'
      'WHERE'
      '  KEY = :Old_KEY')
    SQLUpdate.Strings = (
      'UPDATE AUX_SPECIFIC_CIRCUMSTANCE'
      'SET'
    ...
    SQLRefresh.Strings = (
        ...
        'SELECT IS_DEFAULT, DESCRIPTION, KEY FROM AUX_SPECIFIC_CIRCUMSTAN' +
        'CE'
      'WHERE'
      '  KEY = :KEY')
    SQLLock.Strings = (
      'SELECT NULL FROM AUX_SPECIFIC_CIRCUMSTANCE'
      'WHERE'
      'KEY = :Old_KEY'
      'FOR UPDATE WITH LOCK')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM ('
      'SELECT 1 AS C  FROM AUX_SPECIFIC_CIRCUMSTANCE'
      ''
      ') q')
