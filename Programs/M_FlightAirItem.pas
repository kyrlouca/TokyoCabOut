...
uses
  ...
  vcl.wwcheckbox;
type
  TM_FlightAirwaybillItemFRM = class(TForm)
    Panel1: TPanel;
    Panel4: TPanel;
    Panel2: TPanel;
    Panel3: TPanel;
...
var
  M_FlightAirwaybillItemFRM: TM_FlightAirwaybillItemFRM;
...
procedure TM_FlightAirwaybillItemFRM.BitBtn2Click(Sender: TObject);
begin
if TableSQL.State in [dsEdit,dsInsert] then
  TableSQL.Post;
end;

procedure TM_FlightAirwaybillItemFRM.TableSQLBeforeEdit(
  DataSet: TDataSet);
begin
...
procedure TM_FlightAirwaybillItemFRM.TableSQLNewRecord(DataSet: TDataSet);
begin
  Dataset.FieldByName('status_active').Value:='Y';
  Dataset.FieldByName('CERTIFIED_ANAD').Value:='N';
end;

procedure TM_FlightAirwaybillItemFRM.RzBitBtn1Click(Sender: TObject);
begin
close;
end;

procedure TM_FlightAirwaybillItemFRM.FormActivate(Sender: TObject);
begin
...
procedure TM_FlightAirwaybillItemFRM.FormClose(Sender: TObject;
...
procedure TM_FlightAirwaybillItemFRM.FormCloseQuery(Sender: TObject;
...
procedure TM_FlightAirwaybillItemFRM.FormCreate(Sender: TObject);
begin
  cn:=U_databaseFRM.DataConnection;
end;

procedure TM_FlightAirwaybillItemFRM.Grid1TitleButtonClick(Sender: TObject;
  AFieldName: string);
  var
...
procedure TM_FlightAirwaybillItemFRM.Nav1InsertClick(Sender: TObject);
begin
  FirstFLD.SetFocus;
end;

procedure TM_FlightAirwaybillItemFRM.CanelBTNClick(Sender: TObject);
begin
TableSQL.Cancel;
close;
end;
