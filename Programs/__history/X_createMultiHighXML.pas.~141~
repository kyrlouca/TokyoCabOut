...
type
  ...
  private
    ...
    function CheckSameDestination( Const FlightSerial:integer;XmlRandom:Integer):Boolean;
...
'  select count(*), CONSIGNEE_COUNTRY_CODE from'
  ...
  try
      ...
      qr.ParamByName('FLightSerial').Value:=FlightSerial;
      qr.ParamByName('xmRandom').Value := xmlRandom;
      qr.Open;
      Clipboard.AsText:=qr.FinalSQL;
      result:= qr.RecordCount =1 ;
  finally
...
function TX_CreateMultiHighXmlFRM.CheckSameIncoterm( Const FlightSerial:Integer;XmlRandom:Integer):Boolean;
var
  qr:TksQuery;
  DefaultVal:String;
  str1:string;
