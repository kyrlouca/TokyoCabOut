...
type
  ...
  private
    ...
    Function ProcessFile(Const BatchDetails:TBatchDetails;Var MawbArray: TMawbARray;Var CountMawb,CountHawb:Integer):Boolean;
...
Begin
   ...
   ProcessFile(BatchDetails,mawbArray,CountMawbs,CountHawbs);
...
Function TReadFileFRM.ProcessFile(Const BatchDetails:TBatchDetails;Var MawbArray: TMawbARray;Var CountMawb,CountHawb:Integer):Boolean;
...
//***********************************************************************************
  ...
  If FileExists(BatchDetails.NewFileName) then begin
        DeleteFile(BatchDetails.NewFileName);
  end;

  if not RenameFile(BatchDetails.OldFileName ,BatchDetails.NewFileName) then
        ShowMEssage(' Error - Cannot rename file : '+BatchDetails.OldFileName +' '+IntToStr(GetLastError));
...
Function TReadFileFRM.ReadFilesInFolder:Integer;
Var
        aDirectory:String;
        sr : TSearchRec;
        Count:Integer;
...
//aDirectory:='E:\All\xml';
  aDirectory:= GetValueFromSystemTable('S02', 'Folder to Read XML Files');
  IF (ADirectory='') then begin
        StartBTN.Color:=clRed;
  ...
  if FindFirst(aDirectory+'\'+'*.XML', faAnyFile, SR)=0 then begin
  try
        logFile:=aDirectory+'\'+'LOGFILE.txt';
        ...
        repeat
                ...
                //Newname := Copy(FileFullName, 1, LastDelimiter('.', FileFullName))+'ptf';

                inc(Count);
                FileFullName:=aDirectory+'\'+sr.Name;
                NewName:= changeFileExt(FileFullName,'.txb');

                BatchDetails.OldFileName:=FileFullName;
                BatchDetails.NewFileName:=NewName;
...
procedure TReadFileFRM.Timer1Timer(Sender: TObject);
begin
readFilesinFolder;
end;

procedure TReadFileFRM.fcShapeBtn1Click(Sender: TObject);
var
TimerStatus:Boolean;
begin
//self.ReadAllFiles;
